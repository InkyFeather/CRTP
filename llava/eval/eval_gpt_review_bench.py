...
def get_eval(content: str, max_tokens: int):
    while True:
        try:
            # 创建 Qwen 模型实例，每次调用可动态设置 max_tokens
            qwen_model = ChatOpenAI(
                model="qwen-plus-2025-07-28",
                openai_api_key="sk-47a3eefb41f64a3a8b1ee90cfa17960d",
                openai_api_base="https://dashscope.aliyuncs.com/compatible-mode/v1",
                temperature=0.2,
                # max_tokens=max_tokens,
                # 如果 max_tokens 不生效，可改用 model_kwargs
            )

            messages = [
                SystemMessage(content="You are a helpful and precise assistant for checking the quality of the answer."),
