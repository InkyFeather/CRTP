...
class CLIPVisionTower(nn.Module):
    ...
    def token_reduction(self, image_features, all_image_features, text_features=None):
        ...
        # ========== 判断模式 ==========
        if 'VSG-TRIM' in self.token_reduce_func:
            mode = 'VSG-TRIM'
            k_str = self.token_reduce_func.replace('VSG-TRIM:', '')
        elif 'TRIM' in self.token_reduce_func:
            mode = 'TRIM'
            k_str = self.token_reduce_func.replace('TRIM:', '')
        else:
        ...
        if mode == 'TRIM':
            ...
            # ====== VSG-TRIM: 构建图 + PPR ======
            def build_vsg_and_compute_ppr(proj_img, text_feat, k_nn=6):  # k_nn可调参
                N, D = proj_img.shape
                device = proj_img.device

                # 节点: [N patches + 1 text]
                nodes = torch.cat([proj_img, text_feat.unsqueeze(0)], dim=0)  # [N+1, D]
                sim_matrix = F.cosine_similarity(nodes.unsqueeze(1), nodes.unsqueeze(0), dim=-1)  # [N+1, N+1]

                # KNN 稀疏化视觉-视觉边
                if k_nn > 0 and k_nn < N:
                    vis_sim = sim_matrix[:N, :N]
                    _, topk_idx = torch.topk(vis_sim, k=k_nn, dim=1, largest=True)
                    mask = torch.zeros_like(vis_sim, dtype=torch.bool)
                    mask.scatter_(1, topk_idx, True)
                    mask = mask | mask.T
                    sim_matrix[:N, :N] = torch.where(mask, vis_sim, torch.zeros_like(vis_sim))

                # 移除自环
                A = sim_matrix - torch.diag_embed(torch.diag(sim_matrix))
                ...
                # Personalized PageRank (text node is last)
                r = torch.zeros(N + 1, device=device)
                r[-1] = 1.0
                ppr = r.clone()
                alpha = 0.85  # alpha可调参
                for _ in range(15):
                    ppr = alpha * (P.T @ ppr) + (1 - alpha) * r
                return ppr[:N]  # [N]

            ppr_list = []
            for b in range(batch_size):
                ppr_score = build_vsg_and_compute_ppr(proj_image_features[b], text_features[b], k_nn=6)
                ppr_list.append(ppr_score)
            similarities = torch.stack(ppr_list, dim=0)  # [B, N]
            similarities = -similarities
            similarities = F.softmax(similarities, dim=-1)  # [B, N]
            # PPR 已是概率分布，无需 softmax；但可加温度使其更 sharp（可选）
            # similarities = F.softmax(similarities / 0.5, dim=-1)
